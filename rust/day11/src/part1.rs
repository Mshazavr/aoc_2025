...
impl DAG {
    fn from(text: &str) -> Self {
        ...
        let source_idx = node_names_to_idx_map["you"];
    ...
    fn compute_num_paths_rec(&self, node_idx: usize, num_paths_into: &mut Vec<Option<usize>>) {
        ...
        for neighbour in self.edges_rev[node_idx].iter() {
            if num_paths_into[*neighbour].is_none() {
                self.compute_num_paths_rec(*neighbour, num_paths_into);
            }
    ...
    fn get_num_paths(&self) -> usize {
        ...
        self.compute_num_paths_rec(self.destination_idx, &mut num_paths_into);
        num_paths_into[self.destination_idx].unwrap()
    }
}
