...
fn get_component_representative_id(node: usize, representative_ids: &mut Vec<usize>) -> usize {
    if representative_ids[node] != node {
        representative_ids[node] =
            get_component_representative_id(representative_ids[node], representative_ids);
    }
    representative_ids[node]
}
...
fn merge_components(
    ...
    representative_ids: &mut Vec<usize>,
) {
...
fn get_the_x_product_of_the_critical_connection(boxes: &Vec<JunctionBox>) -> i64 {
    ...
    for (_, (junction_box1_idx, junction_box2_idx)) in distances {
        let representative_node1 =
            get_component_representative_id(junction_box1_idx, &mut representative_ids);
        let representative_node2 =
            get_component_representative_id(junction_box2_idx, &mut representative_ids);
        if representative_node1 == representative_node2 {
            continue;
        }

        merge_components(
            representative_node1,
            representative_node2,
            &mut component_sizes,
            &mut representative_ids,
        );
        num_components -= 1;
        if num_components == 1 {
