...
pub fn is_invalid_id(id: i64) -> bool {
    ...
    for block_size in 1..((num_digits / 2) + 1) {
        if num_digits % block_size != 0 {
            continue;
        }
        let num_blocks: i64 = num_digits / block_size;
        let block_magnitude = id % (10_i64.pow(block_size as u32));
        let composition: i64 = (0..num_blocks)
            .map(|block_idx| block_magnitude * 10_i64.pow((block_size * block_idx) as u32))
            .sum();
...
fn get_invalid_id_sum_in_range(range: Range) -> i64 {
    (range.left..(range.right + 1))
        .map(|x| if is_invalid_id(x) { x } else { 0 })
        .sum()
}
...
pub fn run(run_config: &RunConfig) -> i64 {
    ...
    let result = input
        .into_iter()
        .map(|range| get_invalid_id_sum_in_range(range))
        .sum();
    result
}
